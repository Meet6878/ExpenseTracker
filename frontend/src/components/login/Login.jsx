...
const Login = () => {
  ...
  const handleLoginWithGoogle = () => {
    window.open("http://localhost:7000/google/auth", "_self");
  };

  const getUser = async () => {
    try {
      const response = await axios.get(
        "http://localhost:7000/api/v1/user/login/success",
      ...
      if (response?.data?.user) {
        ...
        var userPic =
          ...
          "https://img.freepik.com/premium-vector/influencer-icon-vector-image-can-be-used-digital-nomad_120816-263441.jpg?w=826";
        setPic(userPic);
        setLoading(false);
        naviage("/expense")
      }
